...
def establish_session(config: dict):
    if has_auth(config):
        ...
        LG.info("Extension -> %s:%s", __name__, establish_session.__name__)
        LG.info("Connecting to -> %s", config["target"])
        ...
        try:
            ...
            while connected:
                ...
                LG.info("%s stdout: %s", rcmd, clean_output(stdout.readlines()))
                LG.error("%s stderr: %s", rcmd, clean_output(stderr.readlines()))
        except Exception as e:
            print(f"Failed to connect: {e}")
            LG.error("Failed to connect: %s", e)
        target.close()
    else:
...
def run_command(rcmd: str, config: dict) -> dict:
    print(f"Extension -> {__name__}:{establish_session.__name__}")
    LG.info("Extension -> %s:%s", __name__, run_command.__name__)
